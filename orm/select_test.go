...
func TestSelector_Build(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			// From 都不调用
			name: "no from",
			q:    NewSelector[model.TestModel](db),
			wantQuery: &Query{
		...
		},
		{
			// 调用 FROM
			name: "with from",
			q:    NewSelector[model.TestModel](db).From("`test_model_t`"),
			wantQuery: &Query{
		...
		},
		{
			// 调用 FROM，但是传入空字符串
			name: "empty from",
			q:    NewSelector[model.TestModel](db).From(""),
			wantQuery: &Query{
				SQL: "SELECT * FROM `test_model`;",
			},
		},
		{
			// 调用 FROM，同时出入看了 DB
			name: "with db",
			q:    NewSelector[model.TestModel](db).From("`test_db`.`test_model`"),
			wantQuery: &Query{
		...
		},
		{
			// 使用 NOT
			name: "not",
			q:    NewSelector[model.TestModel](db).Where(Not(C("Age").GT(18))),
			wantQuery: &Query{
		...
		},
		{
			// 非法列
			name:    "invalid column",
			q:       NewSelector[model.TestModel](db).Where(Not(C("Invalid").GT(18))),
