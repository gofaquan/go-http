...
func TestHandlerBasedOnTree_search(t *testing.T) {
	handler := NewTreeHandler()
	handler.Route(http.MethodPost, "/user", func(c *Context) {})
	_, found := handler.search("/user")
	assert.True(t, found)
	_, found = handler.search("/user/profile")
	...
	handler.Route(http.MethodPost, "/order/detail", func(c *Context) {})
	_, found = handler.search("/order")
	assert.False(t, found)

	_, found = handler.search("/order/detail")
	assert.True(t, found)
